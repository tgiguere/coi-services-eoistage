...
class DapExternalDataHandler(BaseExternalDataHandler):
    ...
    def __init__(self, data_provider=None, data_source=None, ext_data_set=None, *args, **kwargs):
        BaseExternalDataHandler.__init__(self, *args, **kwargs)
        if 'DATASET_URL' in kwargs:
            self.ds_url = kwargs.pop('DATASET_URL')
        elif (data_source is not None and "base_data_url" in data_source) and (ext_data_set is not None and "dataset_path" in ext_data_set):
            self.ds_url = data_source["base_data_url"] + ext_data_set["dataset_path"]
        else:
            raise Exception("Cannot construct dataset url")

        self.ds = Dataset(self.ds_url)
#       self.ds = open_url(self.ds_url)
        self.base_initialize(data_provider, data_source, ext_data_set, **kwargs)
    ...
    def acquire_new_data(self, request=None, **kwargs):
        ...
        tvar = self.ds.variables[self.ext_data_set_object.temporal_id]
...
#        ret["times"] = tvar[sti:eti]
        for vk in self.ds.variables:
            ...
            t_idx = -1
            if self.ext_data_set_object.temporal_id in var.dimensions:
                t_idx = var.dimensions.index(self.ext_data_set_object.temporal_id)
